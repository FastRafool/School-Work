...
def run_algorithm():
    ...
    for i, individual in enumerate(best_individuals):
        ...
        result += f"Average daily protein (g): {total_protein / 7:.2f}\n"
        result += f"Average daily calories: {total_calories / 7:.2f}\n"
        result += f"Average daily fat (g): {total_fat / 7:.2f}\n"
        result += f"Average daily sodium (mg): {total_sodium / 7:.2f}\n"
        result += f"Average daily carbohydrates (g): {total_carbs / 7:.2f}\n"
        result += f"Total weekly cost ($): {total_price:.2f}\n"
